...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (isDemo) {
      return NextResponse.json({ collection: { id, ...body }, demo: true, message: 'Simulated update in demo mode' });
    }
    ...
    const endpoint = `${baseUrl}/admin/api/2024-01/collections/${id}.json`;
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: `Failed to update collection: ${response.status}`, details: errorText }, { status: response.status });
    }
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      return NextResponse.json({ error: `Failed to delete collection: ${response.status}`, details: errorText }, { status: response.status });
    }
